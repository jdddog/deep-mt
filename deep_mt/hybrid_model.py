...
# default parameters are actually densenet121 params
class Densenet121TabularHybrid(nn.Module):
    def __init__(
        self,
        spatial_dims: int,
        in_channels: int,
        ...
        **kwargs,
        ...
        for i, num_layers in enumerate(block_config):
            ...
            self.img_features.add_module(f"denseblock{i + 1}", block)
            in_channels += num_layers * growth_rate
            if i == len(block_config) - 1:
                self.img_features.add_module(
                    "norm5", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=in_channels)
                )
            else:
                _out_channels = in_channels // 2
                ...
                self.img_features.add_module(f"transition{i + 1}", trans)
                in_channels = _out_channels
        ...
        self.class_layers = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("pool", avg_pool_type(1)),
                    ("flatten", nn.Flatten(1)),
                    ("reduced_dim", nn.Linear(1024, 50)),
                ]
            )
        )

        n_cat_layers = 50 + n_tab_features  # in_channels + n_tab_features
        self.out_layer = nn.Sequential(OrderedDict([("out", nn.Linear(n_cat_layers, out_channels))]))
        ...
        # Tabular data
        n_tab_filters = 10
        self.tab_features = torch.nn.Sequential(
            OrderedDict(
                [
                    ("tab_lin1", nn.Linear(n_tab_features, n_tab_filters)),  # tune these
                    ("tab_relu1", nn.ReLU()),
                    ("tab_lin2", nn.Linear(n_tab_filters, n_tab_filters)),
                    ("tab_relu2", nn.ReLU()),
                    ("tab_lin3", nn.Linear(n_tab_filters, n_tab_features)),
                    ("tab_relu3", nn.ReLU()),
                    ("tab_flatten", nn.Flatten(1)),
                ]
            )
        )

        self.aux_out = nn.Sequential(OrderedDict([("aux_out", nn.Linear(n_tab_features, out_channels))]))

    def forward(self, x: torch.Tensor, tab_data: torch.Tensor) -> torch.Tensor:
        x_img = self.img_features(x)
        x_tab = self.tab_features(tab_data)
        x_img = self.class_layers(x_img)

        x_out = torch.cat((x_img, x_tab), dim=1)
        x_out = self.out_layer(x_out)

        # aux_out = self.aux_out(x_tab)
